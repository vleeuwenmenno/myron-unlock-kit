...
echo "Reading current recovery_b before restore..."
current_sha=$(sha256sum "$TARGET" | cut -d ' ' -f 1)
[ "$current_sha" = "$TWRP_SHA" ] ||
  fail "live recovery_b does not match the reviewed TWRP image"

echo "Restoring exact stock recovery_b..."
dd if="$STOCK" of="$TARGET" bs=4194304
sync

echo "Reading recovery_b back for verification..."
readback_sha=$(sha256sum "$TARGET" | cut -d ' ' -f 1)
if [ "$readback_sha" != "$STOCK_SHA" ]; then
  echo "Stock readback failed; restoring the verified TWRP image..." >&2
  dd if="$TWRP" of="$TARGET" bs=4194304
  sync
  restored_sha=$(sha256sum "$TARGET" | cut -d ' ' -f 1)
  [ "$restored_sha" = "$TWRP_SHA" ] ||
    fail "automatic TWRP restoration also failed"
  fail "stock restore failed; TWRP was restored"
fi

echo "Exact stock recovery_b restore and full-partition readback verified: $readback_sha"
